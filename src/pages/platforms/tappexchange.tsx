...
import { initTappSDK  } from '@tapp-exchange/sdk'; // Assumed SDK import
...
// Interfaces based on TappExchange SDK
interface ApiTappToken {
  addr: string;
  amount: number;
  img: string;
  reserve: number;
...
interface ApiTappApr {
  feeAprPercentage: number;
  totalAprPercentage: number;
}

interface ApiTappVolumeData {
  volume24h: number;
}

interface ApiTappPool {
  apr: ApiTappApr;
  fee: string;
  feeTier: string;
  poolId: string;
...
const tappApiKey = import.meta.env.VITE_TAPP_API_KEY;
...
const TappExchangePage = () => {
  ...
  const [tokenPagination, setTokenPagination] = useState(1)
  ...
  const fetchAllPools = async () => {
    try {
      ...
      const poolItems: PoolItem[] = allPoolsData.data.map((apiPool) => {
            ...
            <div className="flex gap-x-2">
              {['0.01', '0.05', '0.3', '1', '0.1', '0.25'].map((option: string) => (
                <Button
                  key={option}
                  variant="outline"
                  className={feeIndex === getFeeIndex(option) ? 'bg-primary/80 text-primary-foreground px-4 rounded-md' : 'px-4 rounded-md'}
                  onClick={() => setFeeIndex(getFeeIndex(option))}
                >
                  {option}
                </Button>
              ))}
